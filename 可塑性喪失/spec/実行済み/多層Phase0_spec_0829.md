...
親: [[LoP防止アーキテクチャ_0829]] §6.4 / 状態: **実行済み（2026-08-29）— G0 = LOP_ABSENT。帰趨は末尾バナーと [[多層Phase0結果_0829]]** / 作成 2026-08-29 / 出典チャット: `arch_lop_0829`
...
**実行済み。judgment = `G0 = LOP_ABSENT`**（`results/mlp2_phase0_0829/verdict.csv`）。結果と読みは [[多層Phase0結果_0829]]。

- 予測1（`LOP_PRESENT`）= **外れ**
- 予測2（第2層の M が第1層より大）= **登録された比較（最終タスク末尾）では外れ**。ただし early 窓では当たっており、時間で逆転
- 予測3（M は増える）= **判定不能**。符号の指定漏れ（下記 1）と CI 退化により、登録した算術が意図と食い違った
- 予測4（外れたら $\Sigma^{(2)}$ の退化を疑う）= **その容疑者は棄却**（NA = 0）。実装差も S0 PASS で棄却

**本 spec の欠陥（後継で修正する。設計者の責任として記録）**

1. **符号の指定漏れ**: $M$ は負の量で壁は $M$ が負に深まるほど強い。§5.2 のトレンドを符号つき $M$ で登録したため、意図（用量の上昇）と算術（符号つき値の増加）が逆を向いた
2. **studentized bootstrap の退化ガードが無い**: 補間 regime では seed 間 SE が潰れて pivot が発散する。`dU` の CI 上端 91.9、トレンドの CI ±10¹³ 台。**`LOP_ABSENT` は実質「late unfit < 0.05」の一本で決まっており、CI の脚は無情報**
3. **regime の入口チェックが無い**: early 窓の未フィット率が v4 の宣言スコープ（≈0.14）から桁で外れていないかを G0 の前段に置くべきだった
4. **判定が seed 平均に依存**: late unfit は seed 間で 14 桁に散り、committed の `late_unfit = 0.00608679` はほぼ seed4 単独で作られている

後継 → [[多層Phase0b_spec_0829]]
