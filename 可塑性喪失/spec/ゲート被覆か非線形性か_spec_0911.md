...
1. **G1 変異対照の閾値**: §4 の「初期値 W1[0,0] += 1e−3 で最初の錨点の再現が 1e−4 を超えて外れる」は、錨点が t1 の腕では外れ幅が 1.05e−4（LR・CELU1 のスモーク）と境界上にある。1 タスクしか経ていない摂動は 1e−4 のオーダーにしかならないので、閾値は committed の慣例（G1 許容 1e−10 の 100 倍 = **1e−8**）に改め、実測値を provenance に記録する。錨点が t20 の腕（Snake 梯子・LIN）は 20 タスク分の増幅を受ける。
2. **G1 の 2 段記録**: `cnorm_i` は全錨で同じ式（Wt.norm）なので bit 一致（0.0）を `g1_cnorm_maxabs` に記録する。`zbar_i`・`sd_i` は `elu_growth_0909` が別の順序で平均を取っている（8 参照置換の stack → mean 対 逐次加算）ため 2.2e−16 の丸め差が出る。登録の閾値 1e−10 はそのまま。
3. **gcorr の縮退**: LIN のように G が定数のとき相関は 0/0 になる。両側が定数なら 1.0（完全に安定）、片側だけ定数なら 0.0 と定義し、件数を `gcorr_degenerate` に記録する。
4. **G3 の変異対照は錨の有無に依らず seed 0 の全腕に掛ける**（新活性化にも）。
