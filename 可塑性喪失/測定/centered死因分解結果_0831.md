...
3. 境界降下の追跡2本は完了した。[[境界窓細粒度記録結果_0831]] は段階1で `TIMESCALES_NOT_SEPARABLE`（τ_fit=230 step）となり段階2を未実行で失効。[[オラクル中心化結果_0831]] は真の切替499点で `BOTH_CONTRIBUTE`（R=0.5167 [0.4858, 0.5805]）。ただしAexactはタスク可識別性も消すため無交絡なB1/B2分解ではない
