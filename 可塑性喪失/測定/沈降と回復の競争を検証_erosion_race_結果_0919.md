...
初期750更新の固定した初期正側の対では、leakyおよびadaptive Snakeの下向き移動はGELUより大きかったが、上向き移動の増加がそれを上回り、正味の下向き移動は小さかった。この結果は、侵食の蓄積を考える際に、下向き輸送の強さだけでなく、戻る方向の更新がどれだけ維持されるかを扱う必要があることを示す。残った応答経路から下向き更新が続く一方、負側へ移った入力からの修正が弱まる場合には、失われた働きを取り戻しにくくなる、というラチェット像と整合的である。

ただし、微分の低下はその入力由来の上向き・下向き両方の勾配を弱めるため、それ自体を下向きの力の発生原因とはしない。本実験で測った上向き量は前活性の変位であり、正側への再進入や機能的回復を直接測った量でもない。「修正に寄与できる入力・ユニットが減ること」と「残った経路の更新が小さくなること」の寄与は未分離である。回復経路の喪失は、この収支差を説明する機構候補として考察に置く。

**研究上の位置づけ（2026-09-19・Issaの整理）**：この未分離部分は活性化ごとの回復量を説明する補足課題とし、ラチェット問題を再び未解決扱いにする穴には数えない。これは課題の優先順位の判断であり、回復経路の因果が今回の実験で確定したという意味ではない。登録判定は変更せず、この点だけを理由に追加実験を開始しない。
